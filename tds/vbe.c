...
int vbe_set_display(void* addr) {
	unsigned int address = (unsigned int)addr;
	struct reg86u reg86;

	reg86.u.w.ax = 0x4F07; /* VBE get mode info *//* translate the buffer linear address to a far pointer */
	reg86.u.w.cx = PB2BASE(address); /* set a segment base */
	reg86.u.w.dx = PB2OFF(address); /* set the offset accordingly */
	reg86.u.b.bl = 0x00;
	reg86.u.b.intno = 0x10;
