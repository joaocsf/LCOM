...
int timer_set_square(unsigned long timer, unsigned long freq) {
	if (timer > 2 || freq == 0 || freq > TIMER_FREQ) return 1; //verifica se o timer esta entre 0 e 2;
...
int timer_unsubscribe_int() {
	...
	if(sys_irqrmpolicy(&hookID) == OK)	return 0;
	return 1;
}
...
int timer_get_conf(unsigned long timer, unsigned char *st) {
	...
	porta = TIMER_0;
	if(timer == 1) porta = TIMER_1;
	else if (timer == 2) porta = TIMER_2;
...
int timer_test_int(unsigned long time) {
	...
	if(timer_subscribe_int() != OK) return 1;
